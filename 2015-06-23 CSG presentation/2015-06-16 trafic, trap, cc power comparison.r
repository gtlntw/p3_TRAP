...
sim_result <- replicate(n_rep, {
  ...
  result.trap.2g2c <- family.test(data=family_generated_2g2c_subsample, f=risk.variant.id)[7]
  ...
  #only report p.value
  c(result.trafic.2c, result.trap.2g2c, result.cc)
})
## Write out your results to a csv file
result.df <- as.data.frame(t(sim_result))
colnames(result.df) <- c("result.trafic.2c", "result.trap.2g2c", "result.cc")
...
result <- read.csv("2015-06-16 trafic, trap, cc power comparison.csv", header=T)
...
filter(result.plot) %>% ggplot(aes(x=r, y=power, group=method, col=method)) +
