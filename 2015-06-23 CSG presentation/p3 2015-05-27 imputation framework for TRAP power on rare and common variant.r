...
sim_result <- replicate(n_rep, {
  ...
  result.trap.nofounder.3c <- family.test.nofounder(data=family_generated_3c, f=risk.variant.id)[7]
  result.trap.2g3c <- family.test.nofounderphenotype(data=family_generated_2g3c, f=risk.variant.id)[7] 
  
  #only report p.value
  c(result.trap.impute.3c, result.trap.nofounder.3c, result.trap.2g3c)
})
## Write out your results to a csv file
result.df <- as.data.frame(t(sim_result))
colnames(result.df) <- c("result.trap.impute.3c", "result.trap.nofounder.3c", "result.trap.2g3c")
...
#############################################
#Load results 
result <- read.csv("p3 2015-05-27 imputation framework for TRAP power on rare and common variant.csv", header=T)
result <- result %>% melt(id.vars=c("seed", "r", "p_dis", "risk.haplo.f","n_family"), variable.name="method", value.name="p.value")
result.plot <- result %>% group_by(r, method, risk.haplo.f) %>% 
  summarise(n=n(), power=mean(p.value < 2.5*10^-4))
result.plot$method <- factor(result.plot$method, label=c("w/ phase_imputed, unknown founder", "known phase, unknown founder", "known phase, known founder"))

#TRAP presentation plot for common
filter(result.plot, risk.haplo.f==0.202, r<1.5) %>% 
...
#                        breaks=c("result.trap.impute.3c", "result.trap.nofounder.3c"),
#                        labels=c("Imputation", "True Phase"))


#TRAP presentation plot for rare
filter(result.plot, risk.haplo.f==0.0039) %>% 
  ggplot(aes(x=r, y=power, group=method, col=method)) +
  #   geom_point(size=3, alpha=1) +
  geom_line(size=1.2, alpha=0.7) +
  ggtitle("f=0.0039") +
