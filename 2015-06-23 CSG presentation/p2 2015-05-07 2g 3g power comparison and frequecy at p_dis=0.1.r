...
result <- read.csv("p2 2015-05-07 2g 3g power comparison and frequecy at p_dis=0.1.csv", header=T)
result$family.strct <- factor(result$family.strct, levels=c("2g.3a.1u", "3g.3a.4u",
                                                            "3g.2a.5u", "3g.3a.5u"))
result$haplotype_on_affect <- factor(result$haplotype_on_affect)
...
result$method <- factor(result$method, labels = c("TRAP", "TRAFIC_EXT"))
# result <- melt(id.vars=c("seed", "r", "p_dis","rish.haplo.f","n_family", ""), variable.name="", value.name="p.value")
...
#CSG presentation
ggplot(result.plot, aes(x=r, y=power, col=family.strct)) +
  facet_wrap(~method) +
  #   geom_point(size=3, alpha=1) +
  geom_line(size=1.2, alpha=0.7) +
  ggtitle("f=0.0102") +
