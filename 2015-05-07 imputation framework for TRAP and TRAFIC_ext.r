...
sim_result <- replicate(n_rep, {
  ...
  result.trap.impute.3c <- family.test.nofounder.impute(data=family_generated_3c, f=1, method="trap")
  result.trap.impute.noaggregate.3c <- family.test.nofounder.impute.noaggregate(data=family_generated_3c, f=1, method="trap")
  result.trafic.impute.3c <- family.test.nofounder.impute(data=family_generated_3c, f=1, method="trafic")
  result.trafic.impute.noaggregate.3c <- family.test.nofounder.impute.noaggregate(data=family_generated_3c, f=1, method="trafic")
  
  
  ##below is the baseline power to compare with to see how much power lost
  result.trap.nofounder.3c <- family.test.nofounder(data=family_generated_3c, f=1)
  result.trafic.ext.3c <- family.test.trafic.ext(data=family_generated_3c, f=1)
...
result <- read.csv("2015-05-07 imputation framework for TRAP and TRAFIC_ext.csv", header=T)
...
filter(result.plot, grepl("trap", method)) %>% ggplot(aes(x=r, y=power, group=method, col=method)) +
  ...
  ggtitle("f=0.0332, consider effect size of risk haplotypes, TRAP") +
...
filter(result.plot, grepl("trafic", method)) %>% ggplot(aes(x=r, y=power, group=method, col=method)) +
  #   geom_point(size=3, alpha=1) +
  geom_line(size=1.2, alpha=0.7) +
  ggtitle("f=0.0332, consider effect size of risk haplotypes, TRAFIC") +
  labs(x="relative risk r") +
  theme_gray(base_size = 20)
...
result.plot %>% ggplot(aes(x=r, y=power, group=method, col=method)) +
  #   geom_point(size=3, alpha=1) +
  geom_line(size=1.2, alpha=0.7) +
  ggtitle("f=0.0332, consider effect size of risk haplotypes, TRAP and TRAFIC") +
  labs(x="relative risk r") +
  theme_gray(base_size = 20)