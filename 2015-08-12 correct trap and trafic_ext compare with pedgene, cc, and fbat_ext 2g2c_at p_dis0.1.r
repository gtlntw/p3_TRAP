...
sim_result <- replicate(n_rep, {
  ...
  family_generated_2g2c_diploid <- hap2dip(data=family_generated_2g2c, risk.variant.id=risk.variant, save.file=T)
  
  ##test based on new imputation framework
  result.trap.2g2c <- family.test(data=family_generated_2g2c, f=risk.variant)$p.value
  result.trafic_ext.2g2c <- family.test.trafic.ext(data=family_generated_2g2c, f=risk.variant)$p.value
...
result <- read.csv("2015-08-12 correct trap and trafic_ext compare with pedgene, cc, and fbat_ext 2g2c_at p_dis0.1.csv", header=T)
...
#see maf on cases and controls
test <- function(r, p_dis){
  prevalence <- p_dis
  b0_sqrt <- sqrt(prevalence)   #baseline
  ...
  family_generated_2g2c <- gene_family(family_strct=family_strct.2g2c, n=n_family, haplotype.risk=haplotype.risk) 
  #maf
  family.maf(data=family_generated_2g2c, f=c(risk.variant.id, 3), case_control=T)
}

sapply(seq(1,3,by=0.1), function(x) test(r=x, p_dis=0.3))
sapply(seq(1,3,by=0.1), function(x) test(r=x, p_dis=0.1))
