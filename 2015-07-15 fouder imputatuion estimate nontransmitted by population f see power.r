...
sim_result <- replicate(n_rep, {
  ...
  result.trap.3c.noimpute <- family.test(data=family_generated_2g3c, f=risk.variant.id, nofounderphenotype=T)$p.value
  result.trap.3c.impute.founder.pop.f <- family.test.trap.impute.founder(data=family_generated_3c, f=risk.variant.id, pop.f=T, pop.f.off=0)
  result.trap.3c.impute.founder.pop.f.off.5 <- family.test.trap.impute.founder(data=family_generated_3c, f=risk.variant.id, pop.f=T, pop.f.off=5)
  result.trap.3c.impute.founder.pop.f.off.10 <- family.test.trap.impute.founder(data=family_generated_3c, f=risk.variant.id, pop.f=T, pop.f.off=10)
  result.trap.3c.impute.founder.pop.f.off.20 <- family.test.trap.impute.founder(data=family_generated_3c, f=risk.variant.id, pop.f=T, pop.f.off=20)
  result.trap.3c.impute.founder.pop.f.off.50 <- family.test.trap.impute.founder(data=family_generated_3c, f=risk.variant.id, pop.f=T, pop.f.off=50)
  
  result.trap.3c.conditional.founderinfo <- family.test(data=family_generated_2g3c, f=risk.variant.id, conditional.test = T)$p.value
  result.trap.3c.conditional.impute.pop.f <- family.test.trap.impute.founder(data=family_generated_3c, f=risk.variant.id, conditional.test=T, pop.f=T, pop.f.off=0)
  ...
  c(result.trap.3c.noimpute, result.trap.3c.impute.founder.pop.f,
    ...
    result.trap.3c.conditional.founderinfo, result.trap.3c.conditional.impute.pop.f)
})
...
result <- read.csv("2015-07-15 fouder imputatuion estimate nontransmitted by population f see power.csv", header=T)
...
filter(result.plot, !grepl("5|10|20|50", method)) %>% ggplot(aes(x=r, y=power, ymax=max(power), group=method, col=method)) +
...
#only trap
pd <- position_dodge(0.0)
filter(result.plot, !grepl("5|10|20|50|conditional", method)) %>% ggplot(aes(x=r, y=power, ymax=max(power), group=method, col=method)) +
  #   geom_point(size=3, alpha=1) +
  facet_wrap(~risk.haplo.f, ncol=3, scale="free_x") +
  geom_line(size=1.2, alpha=0.7, position=pd) +
  geom_point(size=1.2, position=pd) +
