...
sim_result <- replicate(n_rep, {
  ...
  ##test to compare
  result.trap.2g3c <- family.test.nofounderphenotype(data=family_generated_2g3c, f=risk.variant.id)$p.value
  result.trap.weight.2g3c <- family.test.nofounderphenotype(data=family_generated_2g3c, f=risk.variant.id, weight=T)$p.value
...
#   result.cc <- case_control.test(data=data.cc, f=risk.variant)[3]
  ...
  #only report p.value
  c(result.trap.2g3c,result.trap.weight.2g3c)
})
## Write out your results to a csv file
result.df <- as.data.frame(t(sim_result))
colnames(result.df) <- c("result.trap.2g3c","result.trap.weight.2g3c")
...
result <- read.csv("2015-07-02 trap with weight correction.csv", header=T)
