...
risk.variant.id <- c(39) #7 for rare 2 for common 39 for super rare
...
#two versions -- command and rare
# result <- read.csv("2015-06-01 imputation framework for TRAP to fix common variant problem.csv", header=T)
# result <- read.csv("2015-06-01 imputation framework for TRAP on rare.csv", header=T)
result <- read.csv("2015-06-01 imputation framework for TRAP on super rare.csv", header=T)
result <- result %>% melt(id.vars=c("seed", "r", "p_dis","risk.haplo.f","n_family"), variable.name="method", value.name="freq.est")
result.plot <- result %>% group_by(r, method) %>% 
  summarise(n=n(), mu=mean(freq.est), se=sd(freq.est))

#TRAP
pd <- position_dodge(0.05)
result.plot %>% ggplot(aes(x=r, y=mu, group=method, col=method)) +
...
#   ggtitle("f=0.202, consider effect size of risk haplotypes, TRAP") +
  ...
  ggtitle("f=0.078, consider effect size of risk haplotypes, TRAP") +
