...
sim_result <- replicate(n_rep, {
  ...
  result.trap.3c.noimpute <- family.test.nofounderphenotype(data=family_generated_2g3c, f=risk.variant.id)$p.value
  result.trap.3c.impute.founder <- family.test.trap.impute.founder(data=family_generated_3c, f=risk.variant.id)
  result.trap.3c.impute.founder.standardize <- family.test.trap.impute.founder(data=family_generated_3c, f=risk.variant.id, standardize = T)
  result.trap.3c.impute.founder.nontran.true.f <- family.test.trap.impute.founder(data=family_generated_3c, f=risk.variant.id, nontran.true.f=T)
  result.trap.3c.impute.founder.standardize.nontran.true.f <- family.test.trap.impute.founder(data=family_generated_3c, f=risk.variant.id, nontran.true.f=T, standardize = T)
  result.trap.3c.impute.founder.nontran.est.f <- family.test.trap.impute.founder(data=family_generated_3c, f=risk.variant.id, nontran.est.f=T)
  result.trap.3c.impute.founder.standardize.nontran.est.f <- family.test.trap.impute.founder(data=family_generated_3c, f=risk.variant.id, nontran.est.f=T, standardize = T)
  result.trap.3c.impute.conditional.nontran.est.f <- family.test.trap.impute.founder(data=family_generated_3c, f=risk.variant.id, nontran.est.f=T, conditional.test = T)
  result.trap.3c.impute.conditional.standardize.nontran.est.f <- family.test.trap.impute.founder(data=family_generated_3c, f=risk.variant.id, nontran.est.f=T, conditional.test = T, standardize = T)
  
  #only report p.value
  c(result.trap.3c.noimpute,
    result.trap.3c.impute.founder, result.trap.3c.impute.founder.standardize, 
    result.trap.3c.impute.founder.nontran.true.f, result.trap.3c.impute.founder.standardize.nontran.true.f, 
    result.trap.3c.impute.founder.nontran.est.f, result.trap.3c.impute.founder.standardize.nontran.est.f, 
    result.trap.3c.impute.conditional.nontran.est.f, result.trap.3c.impute.conditional.standardize.nontran.est.f)
})
## Write out your results to a csv file
result.df <- as.data.frame(t(sim_result))
colnames(result.df) <- c("result.trap.3c.noimpute",
                         "result.trap.3c.impute.founder", "result.trap.3c.impute.founder.standardize", 
                         "result.trap.3c.impute.founder.nontran.true.f", "result.trap.3c.impute.founder.standardize.nontran.true.f",
                         "result.trap.3c.impute.founder.nontran.est.f", "result.trap.3c.impute.founder.standardize.nontran.est.f", 
                         "result.trap.3c.impute.conditional.nontran.est.f", "result.trap.3c.impute.conditional.standardize.nontran.est.f")
...
system("runslurm.pl -replace -logfile jobs.log -copts \"--time=24:00:00 --mem=512\" jobs.txt")
...
result <- read.csv("2015-07-07 fouder imputatuion estimate nontransmitted power accounted for estimation uncertainty and standardized test.csv", header=T)
colnames(result)[7:15] <- c("result.trap.3c.noimpute",
                            "result.trap.3c.impute.founder", "result.trap.3c.impute.founder.standardize", 
                            "result.trap.3c.impute.founder.nontran.true.f", "result.trap.3c.impute.founder.standardize.nontran.true.f",
                            "result.trap.3c.impute.founder.nontran.est.f", "result.trap.3c.impute.founder.standardize.nontran.est.f", 
                            "result.trap.3c.impute.conditional.nontran.est.f", "result.trap.3c.impute.conditional.standardize.nontran.est.f")
...
filter(result.plot, !grepl("standardize|true", method)) %>% ggplot(aes(x=r, y=power, ymax=max(power), group=method, col=method)) +
...
#compare between standardized vs non-standardized version
pd <- position_dodge(0.0)
filter(result.plot, grepl("est.f", method)) %>% ggplot(aes(x=r, y=power, ymax=max(power), group=method, col=method)) +
  #   geom_point(size=3, alpha=1) +
  facet_wrap(~risk.haplo.f, ncol=3, scale="free_x") +
  geom_line(size=1.2, alpha=0.7, position=pd) +
  geom_point(size=1.2, position=pd) +
