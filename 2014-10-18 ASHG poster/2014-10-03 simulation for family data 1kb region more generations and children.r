...
sim_result <- replicate(n_rep, {
  print(rep.idx)
  rep.idx <- rep.idx + 1
  family_generated <- gene_family(family_strct=family_strct.2g3c, n=500) 
...
(freq <- c(mean((result.cc_null$p.value < alpha)), mean((result.family_null$p.value < alpha))))
xx <- barplot(freq, names=c("case-control", "family 2:2:0"), ylim=c(0,1), ylab="power")
...
par(mfrow=c(1,1), cex=cex, mar=c(3.1, 4.5, 1.6, 0.1), oma=c(0,0,0,0))
n1 <- 1000
alpha=0.05
...
(freq <- c(mean((result.cc$p.value < alpha)), mean((result.family$p.value < alpha))))
xx <- barplot(freq, names=c("Case-Control", "Family"), ylim=c(0,1), ylab="power", cex.lab=cex, cex.axis=cex, cex.main=cex, cex.sub=cex, cex.names=cex)
text(x = xx, y = freq, label=freq, pos = 3, cex = cex, col = "red")
