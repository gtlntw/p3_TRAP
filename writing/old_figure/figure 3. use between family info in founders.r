...
sim_result <- replicate(n_rep, {
  ...
  result.trafic_ext.2g3c.noimpute <- family.test.trafic.ext(data=family_generated_2g3c, f=risk.variant.id)$p.value
  result.trafic_ext.2g3c.association.pop.f <- family.test.trafic.ext(data=family_generated_2g3c, f=risk.variant.id, association.test=T)$p.value
  
  #only report p.value
  c(result.trafic_ext.2g3c.noimpute, result.trafic_ext.2g3c.association.pop.f)
})
## Write out your results to a csv file
result.df <- as.data.frame(t(sim_result))
colnames(result.df) <- c("result.trafic_ext.2g3c.noimpute", "result.trafic_ext.2g3c.association.pop.f")
...
for(i in seq(1,1.3, length.out = 11)) {
...
result.trafic <- read.csv("figure 3. use between family info in founders for trafic_ext.csv", header=T)
result.trafic <- result.trafic %>% melt(id.vars=c("seed", "r", "p_dis","risk.variant","risk.haplo.f","n_family"), variable.name="method", value.name="p.value")
result.combined <- rbind(result, result.trafic)
result.combined.plot <- result.combined %>% group_by(risk.variant, risk.haplo.f, r, method) %>% 
  summarise(n=n(), power=mean(p.value<alpha, na.rm=T))
result.combined.plot$risk.haplo.f <- factor(result.combined.plot$risk.haplo.f, labels=c("f=0.0039","f=0.0178","f=0.202"))
result.combined.plot <- mutate(result.combined.plot, method_kind=ifelse(grepl("trap", method), "TRAP", "TRAFIC_EXT")) 
result.combined.plot$method_kind <- factor(result.combined.plot$method_kind, levels=c("TRAP", "TRAFIC_EXT"), labels = c("TRAP", "TRAFIC_EXT"))
result.combined.plot <- mutate(result.combined.plot, between_family=ifelse(grepl("pop", method), "Yes", "No")) 
# result.combined.plot$between_family <- factor(result.combined.plot$between_family, levels=c("Yes", "No"))
#
pd <- position_dodge(0.0)
filter(result.combined.plot, risk.haplo.f=="f=0.0039") %>% ggplot(aes(x=r, y=power, ymax=max(power), group=method, col=between_family)) +
  #   geom_point(size=3, alpha=1) +
  facet_wrap(~method_kind, ncol=3) +
...
#   ggtitle("f=0.0178, consider effect size of risk haplotypes, TRAP") +
  ...
  coord_cartesian(xlim=c(1,2),ylim = c(0,1.02)) +
