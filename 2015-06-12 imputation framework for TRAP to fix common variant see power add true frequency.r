...
sim_result <- replicate(n_rep, {
  ...
  result.trap.3c.impute.founder.true.f <- family.test.trap.impute.founder(data=family_generated_3c, f=risk.variant.id, true.f=T)
  ...
  result.trap.3c.impute.founder.conditional.true.f <- family.test.trap.impute.founder.conditional(data=family_generated_3c, f=risk.variant.id, true.f=T)
  
  #only report p.value
  c(result.trap.3c.noimpute, result.trap.3c.impute.founder, 
    result.trap.3c.impute.founder.true.f, result.trap.3c.impute.founder.conditional,
    result.trap.3c.impute.founder.conditional.true.f)
})
...
result <- read.csv("2015-06-12 imputation framework for TRAP to fix common variant see power add true frequency.csv", header=T)
