...
n1 <- 1000
sigma2 <- 0
curve(sapply(x, function(x) power.sas(mu=x, sigma2=sigma2, f=0.01, n_sb=n1, alpha=alpha)), 1, 1.3, , col="seagreen3", ylim=c(0,0.001),xlab="",ylab="", lwd=lwd)
mtext("Power", 2, line=2, cex=cex, adj=0.5)
mtext("f=0.01", 3, cex=cex, line=0.2, adj=0.5)
curve(sapply(x, function(x) fn(affect=affect, n_family=n1/2, f=0.01, mu=x, p_dis=0.01, delta=1, alpha=alpha, stat=F)[3]), add=TRUE, lwd=lwd)
...
# grid(NA, NULL)
par(yaxt="n")
curve(sapply(x, function(x) power.sas(mu=x, sigma2=sigma2, f=0.05, n_sb=n1, alpha=alpha)), 1, 4.5, col="seagreen3", ylim=c(0,1), lwd=lwd)
mtext("f=0.05", 3, cex=cex, line=0.2, adj=0.5)
curve(sapply(x, function(x) fn(affect=affect, n_family=n1/2, f=0.05, mu=x, p_dis=0.05, delta=1, alpha=alpha, stat=F)[3]), add=TRUE, lwd=lwd)
...
curve(sapply(x, function(x) power.sas(mu=x, sigma2=sigma2, f=0.2, n_sb=n1, alpha=alpha)), 1, 4.5, col="seagreen3", ylim=c(0,1), lwd=lwd)
mtext("f=0.2", 3, cex=cex, line=0.2, adj=0.5)
curve(sapply(x, function(x) fn(affect=affect, n_family=n1/2, f=0.2, mu=x, p_dis=0.01, delta=1, alpha=alpha, stat=F)[3]), add=TRUE, lwd=lwd)
...
# grid(NA, NULL)
legend("bottomright", c("Family", "Internal Control", "Case-Control"), lty=1, col=c("black", "seagreen3", "darkorange"), cex=1.1, lwd=lwd)
