...
result <- read.csv("2015-06-26 trafic, trafic_ext, trap, cc power comparison.csv", header=T)
...
#square term in logistic traffic
pd <- position_dodge(0.0)
filter(result.plot, grepl("trafic", method)) %>% ggplot(aes(x=r, y=power, ymax=max(power), group=method, col=method)) +
