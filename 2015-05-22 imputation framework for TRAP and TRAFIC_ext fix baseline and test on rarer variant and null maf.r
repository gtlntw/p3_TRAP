...
sim_result <- replicate(n_rep, {
  ...
  #only report p.value
  c(result.trap.impute.3c, 
    result.trap.impute.3c.null,
    result.trap.impute.noaggregate.3c,
    result.trap.impute.noaggregate.3c.null,
    result.trafic.impute.3c,
    result.trafic.impute.3c.null,
    result.trafic.impute.noaggregate.3c,
    result.trafic.impute.noaggregate.3c.null,
    result.trap.nofounder.3c[7],
    result.trafic.ext.3c)
})
...
result <- read.csv("2015-05-22 imputation framework for TRAP and TRAFIC_ext fix baseline and test on rarer variant and null maf.csv", header=T)
...
result.plot <- result %>% group_by(r, method) %>% 
  summarise(n=n(), power=mean(p.value < 2.5*10^-5))
