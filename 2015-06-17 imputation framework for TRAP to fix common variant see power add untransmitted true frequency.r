...
sim_result <- replicate(n_rep, {
  ...
  result.trap.3c.impute.founder.conditional.nontran.true.f <- family.test.trap.impute.founder(data=family_generated_3c, f=risk.variant.id, nontran.true.f=T, conditional.test = T)
  
  #only report p.value
  c(result.trap.3c.noimpute, result.trap.3c.impute.founder, 
    result.trap.3c.impute.founder.nontran.true.f, result.trap.3c.impute.founder.conditional,
    result.trap.3c.impute.founder.conditional.nontran.true.f)
})
...
filter(result.plot) %>% ggplot(aes(x=r, y=power, ymax=max(power), group=method, col=method)) +
  ...
  facet_wrap(~risk.haplo.f, nrow=3) +
