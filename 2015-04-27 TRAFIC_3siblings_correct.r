...
write.csv(data.frame(seed=seed, r=r, p_dis=p_dis, result.trafic.pairwise=sim_result),
          paste("res_",r,"_",n_family,"_",seed,".csv",sep=""), row.names=FALSE)

#plot for the power results adding omnibus test
result <- read.csv("2015-04-27 TRAFIC_3siblings.csv", header=T)
result <- melt(result, id.vars=c("seed", "r","p_dis"), variable.name="method", value.name="p.value")
result.power <- result %>% group_by(r, method) %>% 
  summarise(n=n(), power=mean(p.value<0.05))

##even using the correct 3-sibling family, TRAFic_pairwise stills shows inflated type one error rate