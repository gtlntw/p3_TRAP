...
write.csv(data.frame(seed=seed, r=r, result.2g2c=sim_result[1,], result.trafic.ext=sim_result[2,],result.nofounderphenotype=sim_result[3,], 
                     result.nofounder=sim_result[4,], result.trafic=sim_result[5,]),
          paste("res_",r,"_",n_family,"_",seed,".csv",sep=""), row.names=FALSE)

#plot for the power results adding omnibus test
result <- read.csv("2015-03-27 TRAFIC_ext.csv", header=T)
result.omnibus <- mutate(result, omnibus=pmin(result.2g2c, result.trafic.ext)) %>% select(seed, r, omnibus)
result <- melt(result, id.vars=c("seed", "r"), variable.name="method", value.name="p.value")
result.omnibus <- melt(result.omnibus, id.vars=c("seed", "r"), variable.name="method", value.name="p.value")
result.power <- result %>% group_by(r, method) %>% 
  summarise(n=n(), power=mean(p.value<0.05))
result.power.omnibus <- result.omnibus %>% group_by(r, method) %>% 
  summarise(n=n(), power=mean(p.value<0.025))
result.comb <- rbind(result.power, result.power.omnibus) %>% arrange(r, method) %>% filter(method %in% c("result.2g2c", "result.trafic.ext", "omnibus"))


ggplot(result.comb, aes(x=r, y=power, col=method)) +
