...
sim_result <- replicate(n_rep, {
  ...
  family_generated_2g3c_diploid <- hap2dip(data=family_generated_2g3c, risk.variant.id=c(risk.variant.id, 3), save.file=T)
  
  ##test based on new imputation framework
  result.trap.2g3c <- family.test(data=family_generated_2g3c, f=c(risk.variant.id, 3))$p.value
  result.trafic_ext.2g3c <- family.test.trafic.ext(data=family_generated_2g3c, f=c(risk.variant.id, 3))$p.value
  result.pedgene.2g3c <- pedgene(ped=family_generated_2g3c_diploid$ped, geno=family_generated_2g3c_diploid$geno)
  result.pedgene.vc.2g3c <- result.pedgene.2g3c$pgdf$pval.kernel
  result.pedgene.burden.2g3c <- result.pedgene.2g3c$pgdf$pval.burden
  
  system(paste("./FB-SKAT/FB-SKAT_v2.1 data.ped variant_pass.txt genes.txt weight.txt results_", seed, "_ mendelian_errors_",seed," 10000 0.50 0.01 0 0", sep=""))
  system(paste("./FB-SKAT/FB-SKAT_v2.1 data.ped variant_pass.txt genes.txt weight.txt results_", seed, "_ mendelian_errors_",seed," 10000 0.50 0.01 1 0", sep=""))
  x <- read.table(paste("results_", seed,"_0.500000_0.010000_0.txt", sep="")) #variance component
  result.fbskat.vc.2g3c <- 1-pchisq(x[,4],x[,5])
  x <- read.table(paste("results_", seed,"_0.500000_0.010000_1.txt", sep="")) #burden
  result.fbskat.burden.2g3c=1-pchisq(x[,4],x[,5])
  
  #only report p.value
  c(result.trap.2g3c, result.trafic_ext.2g3c, 
    result.pedgene.vc.2g3c, result.pedgene.burden.2g3c,
    result.fbskat.vc.2g3c, result.fbskat.burden.2g3c)
})
## Write out your results to a csv file
result.df <- as.data.frame(t(sim_result))
colnames(result.df) <- c("result.trap.2g3c", "result.trafic_ext.2g3c", 
                         "result.pedgene.vc.2g3c", "result.pedgene.burden.2g3c",
                         "result.fbskat.vc.2g3c", "result.fbskat.burden.2g3c")
...
system("runslurm.pl -replace -logfile jobs.log -copts \"--time=12:00:00 --mem=2048\" jobs.txt")
...
result <- read.csv("2015-08-03 trap and trafic_ext compare with pedgene and fbat_ext.csv", header=T)
