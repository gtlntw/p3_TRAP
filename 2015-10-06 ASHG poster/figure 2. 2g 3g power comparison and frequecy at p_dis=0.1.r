...
result <- read.csv("figure 2. 2g 3g power comparison and frequecy at p_dis=0.1.csv", header=T)
...
result <- mutate(result, family_structure=family_strct)
result.plot <- result %>% group_by(r, risk.haplo.f, family_structure, method) %>% 
  summarise(n=n(), power=mean(p.value < 2.5*10^-6))
result.plot$risk.haplo.f <- factor(result.plot$risk.haplo.f, labels=c("f=0.0116"))
result.plot$method <- factor(result.plot$method, labels = c("TRAP", "TRAFIC_EXT", "Pedgene.vc", "Pedgene.burden", "FB-SKAT.vc", "FB-SKAT.burden"))

#only TRAP and TRAFIC_EXT withotu 3g.3a.5u
filter(result.plot, grepl("TRAP|TRAFIC_EXT", method), family_structure!="3g.3a.5u") %>% ggplot(aes(x=r, y=power, col=family_structure)) +
  facet_wrap(~method) +
...
ggsave(file="figure 2.png", height=7, width=10)
