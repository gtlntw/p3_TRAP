...
sim_result <- replicate(n_rep, {
  ...
  result.trap.2g3c.noimpute <- family.test(data=family_generated_2g3c, f=risk.variant.id)$p.value
  result.trap.2g3c.association.pop.f <- family.test(data=family_generated_2g3c, f=risk.variant.id, association.test=T)$p.value
  
  #only report p.value
  c(result.trap.2g3c.noimpute, result.trap.2g3c.association.pop.f)
})
## Write out your results to a csv file
result.df <- as.data.frame(t(sim_result))
colnames(result.df) <- c("result.trap.2g3c.noimpute", "result.trap.2g3c.association.pop.f")
...
result <- read.csv("2015-07-20 use between family info in founders.csv", header=T)
