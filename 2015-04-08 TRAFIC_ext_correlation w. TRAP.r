...
ggplot(result.power, aes(x=r, y=power, col=method)) +
  ...
  scale_color_discrete(name="Method", 
                       breaks=c("result.2g2c", "result.trafic.ext", "result.nofounderphenotype", "result.trafic", "result.nofounder"),
                       labels=c("TRAP 2g2c", "TRAFIC 2g2c", "TRAP 2g2c nofounderphenotype", "TRAFIC sibpair", "TRAP sibpair"))

#see the correlation of p.value between TRAP and TRAFIC
result <- read.csv("2015-03-27 TRAFIC_ext.csv", header=T)
result <- result %>% group_by(r)
result %>% summarise(cor(result.2g2c, result.trafic.ext))
ggplot(result, aes(x=result.2g2c, y=result.trafic.ext)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE) +
  facet_wrap(~r, scales="free") +
  labs(x="TRAP", y="TRAFIC") +
  ggtitle("Correlation Coefficint by Relative risk") +
