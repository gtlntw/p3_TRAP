...
sim_result <- replicate(n_rep, {
  ...
  #only report p.value
  c(result.trap.3c.noimpute, result.trap.3c.noimpute.conditional, result.trap.3c.impute.founder)
})
## Write out your results to a csv file
result.df <- as.data.frame(t(sim_result))
colnames(result.df) <- c("result.trap.3c.noimpute", "result.trap.3c.noimpute.conditional", "result.trap.3c.impute.founder")
...
result <- read.csv("2015-06-04 imputation framework for TRAP to fix common variant see power.csv", header=T)
