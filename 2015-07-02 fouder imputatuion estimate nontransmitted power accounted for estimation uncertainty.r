...
sim_result <- replicate(n_rep, {
  ...
  #only report p.value
  c(result.trap.3c.noimpute,result.trap.3c.impute.founder,result.trap.3c.impute.founder.nontran.true.f,result.trap.3c.impute.founder.nontran.est.f)
})
## Write out your results to a csv file
result.df <- as.data.frame(t(sim_result))
colnames(result.df) <- c("result.trap.3c.noimpute","result.trap.3c.impute.founder","result.trap.3c.impute.founder.nontran.true.f","result.trap.3c.impute.founder.nontran.est.f")
...
result <- read.csv("2015-07-02 fouder imputatuion estimate nontransmitted power accounted for estimation uncertainty.csv", header=T)
