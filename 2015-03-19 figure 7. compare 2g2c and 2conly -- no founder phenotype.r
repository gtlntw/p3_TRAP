...
sim_result <- replicate(n_rep, {
  ...
  #only report p.value
  c(result.2g2c[7], result.nofounderphenotype[7], result.nofounder[7], result.trafic[2])
})
## Write out your results to a csv file
write.csv(data.frame(seed=seed, r=r, result.2g2c=sim_result[1,], result.nofounderphenotype=sim_result[2,], 
                     result.nofounder=sim_result[3,], result.trafic=sim_result[4,]),
          paste("res_",r,"_",n_family,"_",seed,".csv",sep=""), row.names=FALSE)

#plot for the results 
result <- read.csv("2015-03-19 figure 7. compare 2g2c and 2conly -- no founder phenotype.csv", header=T)
...
ggplot(result.power, aes(x=r, y=power, col=method)) +
  ...
  scale_color_discrete(name="Method", 
                       breaks=c("result.2g2c", "result.nofounderphenotype", "result.trafic", "result.nofounder"),
                       labels=c("TRAP 2g2c", "TRAP 2g2c nofounderphenotype", "TRAFIC sibpair", "TRAP sibpair"))
